...
export function Confirmation({ bookingData }: ConfirmationProps) {
  ...
  return (
    <div className="text-center">
      {/* Success Icon */}
      <div className="w-20 h-20 bg-[#5b7a52] rounded-full flex items-center justify-center mx-auto mb-6 shadow-sm">
        <svg
      ...
      <h2 className="text-2xl font-playfair font-medium text-stone-900 mb-2">You&apos;re All Set!</h2>
      <p className="text-stone-600 mb-8">
        We&apos;ve sent a confirmation email to{' '}
        <span className="font-medium text-[#8a5a2b]">{bookingData.clientInfo?.email}</span>
      </p>

      {/* Appointment Card */}
      <div className="bg-white backdrop-blur rounded-xl border border-[#e7ddcd] shadow-sm p-6 text-left mb-6">
        <h3 className="font-semibold text-stone-800 mb-4">Appointment Details</h3>

        <div className="space-y-4">
          {/* Date & Time */}
          <div className="flex items-start gap-3">
            <div className="w-10 h-10 bg-[#f4e9d6] rounded-lg flex items-center justify-center flex-shrink-0">
              <svg className="w-5 h-5 text-[#b08344]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="font-medium text-stone-800">{bookingData.service?.name}</p>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-stone-600">Your Stylist</p>
            </div>
          </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="font-medium text-stone-800">KeLatic Hair Lounge</p>
              <p className="text-stone-600">9430 Richmond Ave, Houston, TX 77063</p>
            </div>
          </div>
        </div>

        {/* Price Summary */}
        <div className="mt-6 pt-4 border-t border-[#e7ddcd]">
          <div className="flex justify-between text-sm text-stone-600 mb-1">
            <span>Service Total</span>
            <span>{formatCurrency(totalPrice * 100)}</span>
          </div>
          {depositPaid > 0 && (
            <div className="flex justify-between text-sm text-[#5b7a52] mb-1">
              <span>Deposit Paid</span>
              <span>-{formatCurrency(depositPaid * 100)}</span>
            </div>
          )}
          <div className="flex justify-between font-semibold text-stone-800 mt-2 pt-2 border-t border-[#e7ddcd]">
            <span>Balance Due at Appointment</span>
            <span className="text-[#8a5a2b]">{formatCurrency(balanceDue * 100)}</span>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-3">
        <button
          ...
          className="w-full py-3 bg-[#b08344] text-white rounded-full font-bold hover:bg-[#9a6f33] shadow-sm hover:shadow-md transition-all flex items-center justify-center gap-2"
        >
        ...
        {bookingData.appointmentId ? (
          <Link
            ...
            className="block w-full py-3 bg-white border border-[#e0d4c0] text-stone-700 rounded-full font-semibold hover:bg-[#f3ede3] hover:text-[#8a5a2b] transition-colors text-center"
          >
      ...
      {/* Reminders */}
      <div className="mt-8 bg-[#f4e9d6] border border-[#e0d4c0] rounded-xl p-4 text-left">
        <h4 className="font-semibold text-[#8a5a2b] mb-2">Before Your Appointment</h4>
      ...
      <p className="text-sm text-stone-500 mt-6">
        ...
        <a href="tel:7134854000" className="text-[#8a5a2b] hover:underline">
          (713) 485-4000
        </a>
      </p>
    </div>
