  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <RefreshCw className="w-8 h-8 animate-spin text-[#00ffb2]" />
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-background text-white p-6">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold bg-gradient-to-r from-[#00ffb2] to-orange-500 bg-clip-text text-transparent">
            Command Center
          </h1>
          <p className="text-muted-foreground mt-1">Board of Directors AI Orchestration</p>
        </div>
        <div className="flex gap-3">
          <button
            onClick={() => fetchData()}
            className="flex items-center gap-2 px-4 py-2 bg-muted hover:bg-muted rounded-lg transition"
          >
      ...
      <div className="grid grid-cols-4 gap-4 mb-8">
        ...
        <div className="bg-card rounded-xl p-4 border border-border">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-muted-foreground text-sm">Businesses</p>
        ...
        <div className="bg-card rounded-xl p-4 border border-border">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-[#00ffb2]/20 rounded-lg">
              <DollarSign className="w-5 h-5 text-[#00ffb2]" />
            </div>
            <div>
              <p className="text-muted-foreground text-sm">Monthly MRR</p>
        ...
        <div className="bg-card rounded-xl p-4 border border-border">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-muted-foreground text-sm">Decisions Today</p>
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-4 gap-4">
          {data.orchestrator.agents.map((agent) => {
            ...
            const colorClass = agentColors[agent.role] || 'bg-muted';

            return (
              <div
                key={agent.id}
                className="bg-card rounded-xl p-4 border border-border hover:border-border transition"
              >
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">{agent.role.toUpperCase()}</p>
                    </div>
                  </div>
                  <div className={`px-2 py-1 rounded text-xs ${
                    agent.status === 'active' ? 'bg-green-500/20 text-green-400' :
                    agent.status === 'idle' ? 'bg-muted/20 text-muted-foreground' :
                    agent.status === 'paused' ? 'bg-[#00ffb2]/20 text-[#00ffb2]' :
                ...
                <div className="flex gap-2 mt-4">
                  {agent.status === 'active' ? (
                    <button
                      ...
                      className="flex-1 px-3 py-1.5 bg-muted hover:bg-muted rounded text-xs transition"
                    >
                      Pause
                    </button>
                  ) : (
                    <button
                      ...
                      className="flex-1 px-3 py-1.5 bg-[#00ffb2] hover:brightness-95 rounded text-xs transition"
                    >
                      Start
                    </button>
                  )}
          ...
          <div className="p-4 space-y-3 max-h-80 overflow-y-auto">
            {data.pendingApprovals.length === 0 ? (
              ...
              data.pendingApprovals.map((decision) => (
                <div key={decision.id} className="bg-muted rounded-lg p-3">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-xs text-muted-foreground">{decision.agentRole.toUpperCase()}</span>
                  ...
                  <p className="text-xs text-muted-foreground mb-3">{decision.reasoning}</p>
              ...
              data.recentDecisions.slice(0, 10).map((decision) => (
                <div key={decision.id} className="bg-muted rounded-lg p-3">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-xs text-muted-foreground">{decision.agentRole.toUpperCase()}</span>
                    <span className={`text-xs ${decision.approved ? 'text-green-400' : 'text-[#00ffb2]'}`}>
            ...
            {data.activeAlerts.map((alert: any) => (
              <div
                ...
                className={`p-3 rounded-lg ${
                  ...
                  alert.severity === 'warning' ? 'bg-[#00ffb2]/10 border border-[#00ffb2]/30' :
